SPQR.adam.GPU <- function(params, X, Y, verbose = TRUE) {
    if(verbose == T)
    ...
    ds <- dataset(
        ...
        initialize = function(indices) {
            self$x <- torch_tensor(X[indices,,drop = FALSE], device = 'cuda')
            self$y <- torch_tensor(Y[indices], device = 'cuda')
        },
    ...
    if (params$method == "MAP") {
        model <- nn_SPQR_MAP_GPU(V, # MAP estimation using one of the three priors
                             params$dropout, 
                             params$batchnorm, 
                             params$activation, 
                             params$prior,
    ...
    Btrain <- torch_tensor(Btotal[train_indices,], device = 'cuda')
    Bvalid <- torch_tensor(Btotal[valid_indices,], device = 'cuda')
    ...
    # load best model
    best.model <- torch_load(save_name)$cpu()
...
cv.SPQR.adam.GPU <- function(params, X, Y, folds, verbose) {
...
nn_SPQR_MAP_GPU <- nn_module(
    ...
    initialize = function(V, dropout, batchnorm, activation, prior_class, 
        ...
        if (prior_class == "GP") {
            self$fc[[1]] <- 
                nn_BayesLinear_GP_GPU(V[1], V[2], a_tau, b_tau, FALSE)
            self$fc[[1]]$to(device = 'cuda')
        } else if (prior_class == "ARD") {
            self$fc[[1]] <- 
                nn_BayesLinear_ARD_GPU(V[1], V[2], a_tau, b_tau)
            self$fc[[1]]$to(device = 'cuda')
        } else {
            self$fc[[1]] <- 
                nn_BayesLinear_GSM_GPU(V[1], V[2], a_tau, b_tau, a_kappa, b_kappa)
            self$fc[[1]]$to(device = 'cuda')
        }
        ...
        if (self$layernum > 1) {
            ...
            for (l in 2:self$layernum) {
                if (prior_class == "GSM") {
                    self$fc[[l]] <- 
                        nn_BayesLinear_GSM_GPU(V[l], V[l+1], a_tau, b_tau, a_kappa, b_kappa)
                    self$fc[[l]]$to(device = 'cuda')
                } else {
                    self$fc[[l]] <- 
                        nn_BayesLinear_GP_GPU(V[l], V[l+1], a_tau, b_tau, TRUE)
                    self$fc[[l]]$to(device = 'cuda')
                }
            }
        }
    ...
    forward = function(X) {
        # initialize logprior
        logprior <- torch_tensor(0, device = 'cuda')$sum()
...
nn_BayesLinear_ARD_GPU <- nn_module(
    ...
    forward = function(X) {
        ...
        logprior <- torch_tensor(0,device = 'cuda')$sum()
        logprior$add_(distr_normal(torch_tensor(0,device = 'cuda'),torch_tensor(1,device = 'cuda'))$log_prob(self$W)$sum())
        logprior$add_(distr_gamma(torch_tensor(self$tpa,device = 'cuda'),torch_tensor(self$tpb,device = 'cuda'))$log_prob(tau_W)$sum())
        logprior$add_(distr_normal(torch_tensor(0,device = 'cuda'),torch_tensor(1,device = 'cuda'))$log_prob(self$b)$sum())
        logprior$add_(distr_gamma(torch_tensor(self$tpa,device = 'cuda'),torch_tensor(self$tpb,device = 'cuda'))$log_prob(tau_b)$sum())
        return(list(output=output, logprior=logprior))
    }
)

nn_BayesLinear_GP_GPU <- nn_module(
    classname = "nn_BayesLinear",
    initialize = function(in_features, out_features, a_tau, b_tau, 
    ...
    forward = function(X) {
        ...
        logprior$add_(distr_gamma(torch_tensor(self$tpa, device = 'cuda'),
                                  torch_tensor(self$tpb$divide(self$H), device = 'cuda'))$log_prob(tau_W)$sum())
...
nn_BayesLinear_GSM_GPU <- nn_module(
    classname = "nn_BayesLinear",
    initialize = function(in_features, out_features, a_tau, b_tau,
                          a_kappa, b_kappa) {
    ...
    forward = function(X) {
        ...
        logprior <- torch_tensor(0, device = 'cuda')$sum()
        logprior$add_(distr_normal(torch_tensor(0, device = 'cuda'),torch_tensor(1, device = 'cuda'))$log_prob(self$W)$sum())
        logprior$add_(distr_gamma(torch_tensor(self$kpa, device = 'cuda'),torch_tensor(self$kpb, device = 'cuda'))$log_prob(kappa_W)$sum())
        logprior$add_(distr_normal(torch_tensor(0, device = 'cuda'),torch_tensor(1, device = 'cuda'))$log_prob(self$b)$sum())
        logprior$add_(distr_gamma(torch_tensor(self$kpa, device = 'cuda'),torch_tensor(self$kpb, device = 'cuda'))$log_prob(kappa_b)$sum())
        logprior$add_(distr_gamma(torch_tensor(self$tpa, device = 'cuda'),torch_tensor(self$tpb, device = 'cuda'))$log_prob(tau)$sum())
        return(list(output=output, logprior=logprior))
    }
)
